...
const SHEET_SECTIONS = [
  ...
  },
  {
    ...
    icon: "具",
    title: "Inventário",
    description: "Registro complementar de itens, armas, equipamentos, técnicas, provas, implantes, contratos, recompensas e recursos do personagem."
  },
  {
    id: "activities",
    icon: "任",
...
function buildInitialSheet(character) {
  ...
  if (!Array.isArray(sheet.inventoryRecords)) sheet.inventoryRecords = [];
...
export default function CharacterFullSheetPanel({ character, onCharacterUpdated }) {
  ...
  function updateInventoryDraft(field, value) {
  ...
  function addInventoryRecord() {
    const title = inventoryDraft.title.trim();
    const category = String(inventoryDraft.category || "Item").trim();
    const quantity = String(inventoryDraft.quantity || "1").trim();
    const description = inventoryDraft.description.trim();

    if (!title) {
      setMessage("Informe o nome do registro de inventário.");
      return;
    }
    ...
    setSheet((current) => ({
      ...
      inventoryRecords: [
        record,
        ...(Array.isArray(current.inventoryRecords) ? current.inventoryRecords : [])
      ]
    }));
    ...
    setMessage("Registro de inventário adicionado. Clique em “Salvar ficha” para gravar.");
  }

  function removeInventoryRecord(recordId) {
    setSheet((current) => ({
      ...current,
      inventoryRecords: (Array.isArray(current.inventoryRecords) ? current.inventoryRecords : []).filter(
        (record) => record.id !== recordId
      )
    }));

    setMessage("Registro de inventário removido. Clique em “Salvar ficha” para gravar.");
  }
  ...
  function hasSectionContent(section) {
    ...
    if (section.id === "inventory") {
      return Array.isArray(sheet.inventoryRecords) && sheet.inventoryRecords.length > 0;
    }
  ...
  function renderInventorySection() {
    const records = Array.isArray(sheet.inventoryRecords) ? sheet.inventoryRecords : [];

    return (
      <>
        <div className="academic-proof-editor inventory-editor">
          <div className="academic-proof-editor-title">
            <div>
              <h4>Adicionar registro ao inventário</h4>
              <p>
                Use esta área para registrar itens, armas, técnicas, equipamentos, implantes,
                contratos, recompensas, provas e recursos especiais.
              </p>
            </div>
          </div>

          <div className="academic-proof-form inventory-form">
            <label>
              Categoria
              <input
                value={inventoryDraft.category || ""}
                onChange={(event) => updateInventoryDraft("category", event.target.value)}
                placeholder="Ex.: Item, Arma, Técnica, Equipamento, Implante..."
              />
            </label>
            ...
            <label>
              Nome do registro
              <input
                value={inventoryDraft.title || ""}
                onChange={(event) => updateInventoryDraft("title", event.target.value)}
                placeholder="Ex.: Kunai especial, treino aprovado, contrato..."
              />
            </label>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => updateInventoryDraft("quantity", event.target.value)}
                placeholder="1"
              />
            </label>
            ...
            <label className="academic-proof-form-full">
              ...
              <textarea
                ...
                onChange={(event) => updateInventoryDraft("description", event.target.value)}
                placeholder="Ex.: Origem do item, missão, compra, treino, aprovação do ADM, print relacionado ou observação importante."
              />
            </label>

            <button type="button" onClick={addInventoryRecord}>
              Adicionar ao inventário
            </button>
          </div>
        </div>

        <div className="academic-proof-list inventory-list">
          <h4>Inventário registrado</h4>

          {records.length === 0 ? (
            <p className="academic-proof-empty">
              Nenhum registro de inventário salvo.
            </p>
          ) : (
            <div className="inventory-record-grid">
              {records.map((record) => (
                <article key={record.id} className="inventory-record-card">
                  <button
                    type="button"
                    onClick={() => removeInventoryRecord(record.id)}
                    aria-label="Remover registro de inventário"
                  >
                    ×
                  </button>

                  <small>{record.category || "Item"}</small>
                  <strong>{record.title}</strong>

                  <span>Quantidade: {record.quantity || "1"}</span>
  ...
  function renderSectionContent(section) {
    ...
    if (section.id === "inventory") return renderInventorySection();
